				       	...
				       	if(flatResults) {
				       		...
				       		if(flatMap.size()>0) {
					       		...
					       		for(String key:flatMap.keySet()) {
					       			...
					       			%>
					       				<a href="addReaction.do?ID=<%= allergy.getDrugrefId() %>&name=<%=java.net.URLEncoder.encode(StringUtils.abbreviate(allergy.getDescription(),50))%>&type=<%=allergy.getTypeCode()%>">LL<%=allergy.getDescription()%></a>
<%
						...
						<%
							...
							if(allergyResults.get(8).size()>0) {
								...
								for(Allergy allergy:allergyResults.get(8)) {
									%>
									<a href="addReaction.do?ID=<%= allergy.getDrugrefId() %>&name=<%=java.net.URLEncoder.encode(StringUtils.abbreviate(allergy.getDescription(),50))%>&type=<%=allergy.getTypeCode()%>"><%=allergy.getDescription()%></a>
									<br/>
									<%
								}
								%></div><%
