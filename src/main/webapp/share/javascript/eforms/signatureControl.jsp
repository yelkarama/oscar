	...
	initialize : function(options) {
		...
		// Generating an eform configuration.
		 if (typeof options == "undefined" || options == null || options.eform) {
			...
			if (jQuery(element.signatureValue).length > 0 && "" != jQuery(element.signatureValue).val()) {
			...
			element.refreshImage =  
				function(e) {
					if (jQuery(element.signatureDisplay).length > 0) {
					...
					if (jQuery(element.signatureValue).length > 0) {
		...
		if ($(element.signatureInput).length > 0) {
