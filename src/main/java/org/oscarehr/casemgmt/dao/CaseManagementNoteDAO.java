    ...
    public List<CaseManagementNote> getActiveNotesByDemographic(String demographic_no, String[] issues) {
		...
		if (issues != null) {
			if (issues.length > 1) {
				...
				// refactored query to do it in one SQL operation to improve speed
				String hqlQueryString = "select cmn from CaseManagementNote cmn join cmn.issues i where i.issue_id = ? and cmn.demographic_no = ? and cmn.archived = 0 and cmn.id = (select max(cmn2.id) from CaseManagementNote cmn2 where cmn.uuid = cmn2.uuid) ORDER BY cmn.position, cmn.observation_date desc";
                List<CaseManagementNote> noteList = this.getHibernateTemplate().find(hqlQueryString, new Object[] { id, demographic_no });
				return noteList;
    ...
    public List<CaseManagementNote> getNotesByDemographic(String demographic_no, String[] issueIds, Integer maxNotes) {
		...
		if (issueIds != null) {
			if (issueIds.length > 1) {
				...
				hql = "select cmn from CaseManagementNote cmn join cmn.issues i where i.issue_id in (" + list
				        + ") and cmn.demographic_no = ? and cmn.id = (select max(cmn2.id) from CaseManagementNote cmn2 where cmn.uuid = cmn2.uuid) order by cmn.position, cmn.observation_date desc ";
				retList = this.getHibernateTemplate().find(hql, demographic_no);

			} else if (issueIds.length == 1) {
				hql = "select cmn from CaseManagementNote cmn join cmn.issues i where i.issue_id = ? and cmn.demographic_no = ? and cmn.id = (select max(cmn2.id) from CaseManagementNote cmn2 where cmn.uuid = cmn2.uuid) order by cmn.position, cmn.observation_date desc";
